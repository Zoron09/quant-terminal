...
def main():
    if len(sys.argv) < 2:
        print("Usage: python batch_c33_scan.py <path_to_csv>")
        sys.exit(1)
    ...
    print(f"Found {len(tickers)} tickers. Starting batch scan...")
    ...
    with open(all_file, 'w', newline='', encoding='utf-8') as f_all, \
        ...
        for i, ticker in enumerate(tickers):
            try:
                ...
                print(f"[{i+1}/{len(tickers)}] Error processing {ticker}: {e}")
                all_writer.writerow({'Symbol': ticker, 'Status': 'ERROR'})
                f_all.flush()
